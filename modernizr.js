    ...
    tests['arraybuffer'] = function() {
        ...
        try {
            if(bool = 'ArrayBuffer' in window) {
                bool = new Boolean(bool);
                bool.dataview = (typeof DataView !== 'undefined' && 'getFloat64' in DataView.prototype);
            }
        } catch(e) {}
        return bool;
    }
...
(function(){
    ...
    xhr.onload = function() {
        if((this.status == 200 || this.status == 0) && this.response) {
            ...
            Modernizr.addTest('xhrblob', function () { return (blob.size == 5); });
        } else {
            Modernizr.addTest('xhrblob', function () { return false; });
        }
    }
    ...
    xhr.onload = function() {
        if((this.status == 200 || this.status == 0) && this.response) {
            ...
            Modernizr.addTest('xhrarraybuffer', function () { return (arr.byteLength == 5); });
        } else {
            Modernizr.addTest('xhrarraybuffer', function () { return false; });
        }
    }
